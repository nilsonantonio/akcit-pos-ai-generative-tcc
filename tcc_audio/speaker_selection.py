"""Select a small balanced speaker subset and build the data manifest."""
...
def select_speakers(
    ...
    for gender, gender_df in rng.groupby("gender"):
        ranked = (
            gender_df.groupby("source_speaker_id", as_index=False)["duration_s"]
            .sum()
            .sort_values("duration_s", ascending=False)
        )
        chosen_speakers = ranked[ranked["duration_s"] >= target_seconds].head(speakers_per_gender)
        for _, speaker in chosen_speakers.iterrows():
            source_speaker_id = speaker["source_speaker_id"]
            speaker_rows = gender_df[gender_df["source_speaker_id"].eq(source_speaker_id)].copy()
...
def main(argv: list[str] | None = None) -> int:
    ...
    manifest, speaker_selection = select_speakers(
        ...
        speakers_per_gender=args.speakers_per_gender,
