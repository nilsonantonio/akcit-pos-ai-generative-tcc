...
def aggregate_metrics(samples_path: str | Path, out_dir: str | Path | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    group_columns = ["condition", "text_variant"]
    ...
    if completed.empty:
        ...
        cost_summary = (
            completed.groupby("condition", dropna=False)
            .agg(
...
def _paired_condition_tests(samples: pd.DataFrame) -> pd.DataFrame:
    """Run paired Wilcoxon tests between core conditions when matching samples exist."""
    core_pairs = [
        ("speecht5_lora", "speecht5_zero_shot"),
        ("speecht5_lora", "speecht5_few_shot_decoder_ft"),
    ]
    rows: list[dict[str, object]] = []
    index_cols = ["speaker_id", "prompt_id", "text_variant"]

    for metric in ["wer", "speaker_similarity", "nisqa", "f0_rmse"]:
        for left_condition, right_condition in core_pairs:
            left = samples[samples["condition"].eq(left_condition)][index_cols + [metric]].rename(
                columns={metric: "left"}
            )
            right = samples[samples["condition"].eq(right_condition)][index_cols + [metric]].rename(
